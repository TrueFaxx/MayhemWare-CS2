...
internal class BombTimer(Graphics.Graphics graphics) : ThreadedServiceBase
{
    ...
    protected override void FrameAction()
    {
        ...
        _timeLeft = Math.Max(_c4Blow - _currentTime, 0);
        _defuseLeft = _beingDefused ? Math.Max(_defuseCountDown - _currentTime, 0) : 0;
        if (_isBombPlanted)
    ...
    public static void Draw(Graphics.Graphics graphics)
    {
        ...
        var defuse = _beingDefused ? $"Defuse time: {_defuseLeft:0.00} seconds" : "Not being defused";
        graphics.FontAzonix64.DrawText(default, $"{_bombPlanted}{Environment.NewLine}{timerText}{Environment.NewLine}{defuse}", 0, 500, Color.WhiteSmoke);
    }
}